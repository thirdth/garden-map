...
export function GardenApp({ session }: Props) {
  ...
  const { plantings, addPlanting, removePlanting, updatePlanting } = usePlantings(currentYard?.id ?? '')
  ...
  function handleOverlayToggle(overlay: 'elevation' | 'waterflow' | 'shade') {
    ...
    if (!shown[overlay]) { setShown[overlay](true); setPaintOverlay(overlay) }
    else if (paintOverlay === overlay) { setShown[overlay](false); setPaintOverlay(null) }
    else { setPaintOverlay(overlay) }
  }

  const overlayButtonClass = (overlay: 'elevation' | 'waterflow' | 'shade', activeColor: string, ringColor: string) => {
    const shown = { elevation: showElevation, waterflow: showWaterFlow, shade: showShade }
    if (!shown[overlay]) return 'text-stone-500 border-stone-200 hover:bg-stone-50'
    if (paintOverlay === overlay) return `${activeColor} ring-2 ${ringColor} ring-offset-1`
      ...
      <header className="bg-white border-b border-stone-200 px-4 py-2.5 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-0.5">
            {(['garden', 'plants'] as AppView[]).map(v => (
              <button
                key={v}
                onClick={() => setView(v)}
        ...
        <>
          {loading ? (
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="flex flex-col flex-1 overflow-hidden min-w-0">
                ...
                {showElevation && (
                  <div className="bg-stone-50 border-b border-stone-200 px-4 py-2 flex items-center gap-3 shrink-0">
                    <ElevationPalette selected={paintElevation}
                      onSelect={v => { setPaintElevation(v); setPaintOverlay('elevation') }} />
                  </div>
                )}
                {showWaterFlow && (
                  <div className="bg-stone-50 border-b border-stone-200 px-4 py-2 flex items-center gap-3 shrink-0">
                    <WaterFlowPalette selected={paintFlow}
                      onSelect={v => { setPaintFlow(v); setPaintOverlay('waterflow') }} />
                  </div>
                )}
                {showShade && (
                  <div className="bg-stone-50 border-b border-stone-200 px-4 py-2 flex items-center gap-3 shrink-0">
                    <ShadePalette selected={paintShade} onSelect={setPaintShade}
                ...
                {showPlants && (
                  <div className="bg-stone-50 border-b border-stone-200 px-4 py-2 flex items-center gap-3 shrink-0 flex-wrap">
                    <PlantSearch selected={selectedPlant}
                      onSelect={p => { setSelectedPlant(p); setSelectedPlanting(null) }} />
                    {selectedPlant && (
                      <span className="text-xs text-green-700 font-medium shrink-0">Click a cell to place</span>
                    )}
                  </div>
                )}
                ...
                {currentYard && (
                  <YardGrid
                    ...
                    paintOverlay={paintOverlay} onPaintCell={handlePaintCell}
                    showPlants={showPlants} plantings={plantings} selectedPlanting={selectedPlanting}
              ...
              {/* Detail panel — shown when a planting is selected */}
              {selectedPlanting && (
                <PlantDetailPanel
                  plantId={selectedPlanting.plant_id}
                  planting={selectedPlanting}
                  onUpdatePlanting={fields => updatePlanting(selectedPlanting.id, fields)}
                  onRemovePlanting={() => { removePlanting(selectedPlanting.id); setSelectedPlanting(null) }}
